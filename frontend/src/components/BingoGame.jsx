...
export default function BingoGame({ room, user, socket, onOpenCardSelector, onLeaveRoom, globalMasterSeconds }) {
  ...
  useEffect(() => {
    ...
    const handleBingoWinner = (data) => {
      ...
      // INSTANT CLEAN EXIT: Clear session storage, reset state, and return to Lobby after 4s!
      setTimeout(() => {
  ...
  // 100% SERVER-AUTHORITATIVE BALL LISTENER
  // All ball draws, voice announcements, and game win events are controlled strictly by server sockets!
  useEffect(() => {
